 ...
 */
class Store extends \Magestore\Storelocator\Model\ResourceModel\AbstractResource
{
    /**
    ...
    {
        $this->_init(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE, 'storelocator_id');
    }
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        $select->joinLeft(
            ['table_schedule' => $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_SCHEDULE)],
            $this->getMainTable() . '.schedule_id = table_schedule.schedule_id'
        )->joinLeft(
            ['table_image' => $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_IMAGE)],
    ...
    public function getTagIds(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_TAG),
            'tag_id'
        )->where(
    ...
    public function getHolidayIds(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_HOLIDAY),
            'holiday_id'
        )->where(
            $this->getIdFieldName() . ' = :object_id'
        );
    ...
    public function getSpecialdayIds(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_SPECIALDAY),
            'specialday_id'
        )->where(
            $this->getIdFieldName() . ' = :object_id'
        );
    ...
    {
        $id = (int) $object->getId();
        $table = $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_TAG);
    ...
    {
        $id = (int) $object->getId();
        $table = $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_HOLIDAY);
    ...
    {
        $id = (int) $object->getId();
        $table = $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_SPECIALDAY);
    ...
    {
        $table = $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_IMAGE);
        ...
        $this->updateData(
            $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE),
    ...
    protected function _getImageIdByPath($path) {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_IMAGE),
            ['image_id']
        )->where(
            'path = :path'
        );
    ...
    public function countHolidays(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $select = $connection->select()->from(
            ['store_holiday' => $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_HOLIDAY)],
            ['COUNT(*)']
        )->joinLeft(
            ['holiday' => $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_HOLIDAY)],
    ...
    public function countSpecialdays(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $select = $connection->select()->from(
            ['store_specialday' => $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_STORE_SPECIALDAY)],
            ['COUNT(*)']
        )->joinLeft(
            ['specialday' => $this->getTable(\Magestore\Storelocator\Setup\InstallSchema::SCHEMA_SPECIALDAY)],
